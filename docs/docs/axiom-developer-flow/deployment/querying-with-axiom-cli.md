...
```bash
cast send --rpc-url $RPC_URL_11155111 --private-key $PRIVATE_KEY_11155111 --value <payment value> 0x83c8c0B395850bA55c830451Cfaca4F2A667a983 <sendQueryCalldata>
```
...
We currently support Ethereum Mainnet, Sepolia, and Base Sepolia. You can simply modify the `sourceChainId` and `rpc-url` CLI arguments to the appropriate values for the chain you want to use. Ensure that the `private-key` that you pass in is also funded on the chain that you are using and that the `<callback addr>` is a valid contract that will accept an Axiom callback.
